class HotelsController < ApplicationController
  ...
  def search
    ...
    @reviews = if reviews_age_eq == 'on'
                 Review.joins(:hotel).merge(@results)
               else
                 Review.joins(:hotel).merge(@results).where(age: reviews_age_eq)
               end
    ...
    @results.each do |hotel|
      ...
      unless detail && detail['hotels'] && detail['hotels'][0] && detail['hotels'][0][0] && detail['hotels'][0][0]['hotelBasicInfo']
        next
      end
  ...
  def show
    ...
    @room_info = check_vacant_rooms(hotel_no, checkin_date, checkout_date, adult_num, infant_with_mb, infant_with_m,
                                    infant_with_b, infant_without_mb)
  end
  ...
  def fetch_hotel_no(hotel_name)
    ...
    if detail && detail['hotels'] && detail['hotels'][0] && detail['hotels'][0][0] && detail['hotels'][0][0]['hotelBasicInfo']
      detail['hotels'][0][0]['hotelBasicInfo']['hotelNo']
    end
  end

  def check_vacant_rooms(hotel_no, checkin_date, checkout_date, adult_num, infant_with_mb, infant_with_m, infant_with_b,
                         infant_without_mb)
